...
from spyderlib.widgets.dicteditorutils import (get_basic_props, make_meta_dict, 
                                                    ndarray)
...
class Monitor(threading.Thread):
    ...
    def __init__(self, host, introspection_port, notification_port,
        ...
        self._mlocals = {
                       ...
                       "get_props_for_variable_explorer":
                                        self.get_props_for_variable_explorer,
    ...
    #------ Other
    def get_props_for_variable_explorer(self, keys_path=None):
        """For each varaible in the current namespace, it produces a "props" 
        dict.  The props dicts are stored in a tuple, with the name of each
        variable stored within the props as "key".  The props dict is
        generated by get_basic_props in dictutils.
                
        ``keys_path`` can be None or it can be a tuple of keys to evalute
        in order to traverse down into a sub "namespace" within the namespace.
        e.g. my_dict[some_sub_dict_name][some_numpy_obj_array][(4,5)][quest]
        """
        ns = self.get_current_namespace() # globals/locals/pdb or something
        ...
        if isinstance(ns, (list, tuple)):
            ...
            raise NotImplementedError # TODO: if ns.dtype == dtype(object)
        elif hasattr(ns, '__iter__'): # this includes dict
            return tuple(get_basic_props(key, ns[key]) for key in ns)
        else:
